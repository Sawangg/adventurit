...
export default async function AdminLoginPage({ params }: { params: { lang: Locale } }) {
  ...
  return (
    <main className="flex h-screen items-center justify-center">
      <Card>
        <CardHeader>
          <CardTitle>{dictionnary.admin.login.connection}</CardTitle>
          <CardDescription>{dictionnary.admin.login.choice}</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-background px-2 text-muted-foreground">{dictionnary.admin.login.division}</span>
            </div>
          </div>
